...
class _ChatRoomScreenState extends State<ChatRoomScreen> {
  ...
  late StreamSubscription blockedRoomSubscription;
  ...
  void initState() {
    ...
    blockedRoomSubscription = ChatService.instance.myRoomsBlockedCol
        .doc(otherUid)
        .snapshots()
  ...
  void dispose() {
    super.dispose();
    blockedRoomSubscription.cancel();
  }
          ...
          UserDoc(
              ...
              builder: (user) {
                return Popup(
                  ...
                  initialValue: '',
                  options: {
                    'friendMap': PopupOption(icon: const Icon(Icons.map), label: 'Friend Map'),
                    if (blocked) 'unblock': PopupOption(icon: Icon(Icons.block), label: 'Unblock'),
                    if (!blocked) 'block': PopupOption(icon: Icon(Icons.block), label: 'Block'),
                  ...
                  onSelected: (v) async {
                    if (v == 'friendMap') {
                      ...
                      final re = await service.confirm('Block', 'Do you want to block this user?');
                      if (re == false) return;
                      // block user
                      ...
                      service.alert('Reported', 'You have been reported this user successfully.');
                    }
                  },
                );
              }),
