...
describe("Comment point test; " + uid, () => {
  it("Comment create event test - uid: " + uid, async () => {
    ...
    lib.randomPoint[lib.pointEvent.commentCreate].within = 3;
    ...
    const ref = await lib.commentCreatePoint({uid: uid}, {params: {commentId: commentId}});
    assert.ok(ref);
    ...
    const updateRef = await lib.commentCreatePoint(
        {uid: uid},
    ...
    const ref2 = await lib.commentCreatePoint(
        {uid: uid},
    ...
    const ref3 = await lib.commentCreatePoint(
        {uid: uid},
