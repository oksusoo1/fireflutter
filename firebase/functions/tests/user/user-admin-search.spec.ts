...
describe("User admin search", async () => {
  it("admin search", async () => {
    ...
    try {
      const result = (await User.adminUserSearch(
        {
          email: "pinedaclp@gmail.com,thruthesky@gmail.com",
      ...
      expect(result.users[0].email).equal("pinedaclp@gmail.com");
      expect(result.users[0].phoneNumber).equal("+639152308483");
    } catch (e) {
      console.log(e);
    }
  });
