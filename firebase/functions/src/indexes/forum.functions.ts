...
export const postCreate = functions.region("us-central1").https.onRequest((req, res) => {
  ready({ req, res, auth: true }, async (data) => {
    res.status(200).send(await Post.create(data));
  });
});

export const postUpdate = functions.region("us-central1").https.onRequest((req, res) => {
  ready({ req, res, auth: true }, async (data) => {
    res.status(200).send(await Post.update(data));
  });
});

export const postDelete = functions.region("us-central1").https.onRequest((req, res) => {
  ready({ req, res, auth: true }, async (data) => {
    res.status(200).send(await Post.delete(data));
  });
});

export const commentCreate = functions.region("us-central1").https.onRequest((req, res) => {
  ready({ req, res, auth: true }, async (data) => {
    res.status(200).send(await Comment.create(data));
  });
});

export const commentUpdate = functions.region("us-central1").https.onRequest((req, res) => {
  ready({ req, res, auth: true }, async (data) => {
    res.status(200).send(await Comment.update(data));
  });
});

export const commentDelete = functions.region("us-central1").https.onRequest((req, res) => {
  ready({ req, res, auth: true }, async (data) => {
